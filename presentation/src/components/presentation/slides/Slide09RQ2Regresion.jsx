...
export default function Slide09RQ2Regresion({ totalSlides, slideNumber }) {
  ...
  const modelsRows = [
    ['M0', 'RemoteWork', '0.016', '—'],
    ['M1', '+ YearsCodePro_num', '0.115', '+0.099'],
    ['M2', '+ EdLevel + DevType', '0.143', '+0.028'],
    ['M3', '+ Country', '0.456', '+0.313'],
  ];

  const coefficientsHeaders = ['Variable', 'β (M3)', 'exp(β) − 1', 'IC 95%', 'p-value'];
  const coefficientsRows = [
    ['Hybrid (ref: In-person)', '0.196839', '+21.75%', '[+17.20%, +26.49%]', '4.71e−24'],
    ['Remote (ref: In-person)', '0.290031', '+33.65%', '[+28.00%, +39.54%]', '1.27e−39'],
    ['YearsCodePro_num', '0.046', '+4.7%/año', 'Editable', '< 0.001'],
  ];
  ...
  return (
    <SlideLayout
      ...
      technicalNote="El ajuste por Country (M3) produce el mayor incremento en R² (+0.313), revelando la heterogeneidad salarial por país como principal confusor. Los coeficientes de Remote/Hybrid disminuyen respecto a M0 pero permanecen significativos, confirmando un efecto ajustado real."
    >
      <div className="space-y-6">
        ...
        <ContentCard title="Modelos Incrementales" icon={Layers}>
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { model: 'M0', r2: '1.6%', label: 'Solo RemoteWork' },
              { model: 'M1', r2: '11.5%', label: '+ Experiencia' },
              { model: 'M2', r2: '14.3%', label: '+ Ed. + DevType' },
              { model: 'M3', r2: '45.6%', label: '+ Country' },
            ].map((m) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ContentCard title="Coeficientes del Modelo Final (M3)" icon={LineChart} variant="accent">
            <DataTable
              ...
              caption="La interpretación porcentual se obtiene como exp(β) − 1. Ejemplo: β = 0.290 → exp(0.290) − 1 ≈ 33.65%"
            />
          </ContentCard>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl border border-emerald-200">
            ...
            <p className="text-3xl font-bold text-emerald-700">+33.6%</p>
            <p className="text-base text-slate-700 mt-1">β ≈ 0.290, ajustado por confusores</p>
          </div>
          <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
            <p className="font-semibold text-blue-800 mb-2">Hybrid (vs In-person)</p>
            <p className="text-3xl font-bold text-blue-700">+21.8%</p>
            <p className="text-base text-slate-700 mt-1">β ≈ 0.197, ajustado por confusores</p>
          </div>
        </div>
      </div>
    </SlideLayout>
